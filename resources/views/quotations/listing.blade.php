                ...
                <div class="border-bottom pt-6">
                    <div class="row align-items-center">
                        <div class="col-sm col-12">
                            <h1 class="h2 ls-tight">Quotations</h1>
                        </div>
                ...
                <div class="vstack gap-4">
                    ...
                    <div class="card">
                        <div class="card-header border-bottom d-flex align-items-center">
                            <h5 class="me-auto">{{ count($quotations->unique('quotation_number')) }} Quotation(s)</h5>
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover table-nowrap">
                                ...
                                <tbody>@foreach ($quotations->unique('quotation_number') as $quotation)
                                    <tr>
                                        <td>
                                            {{ $quotation->buyer->name }}
                                        </td>
                                        <td>
                                            <a href="{{ route('quotations.show', ['quotation' => $quotation->quotation_number]) }}" target="_blank">{{ $quotation->quotation_number }}</a>
                                        </td>
                                        <td>
                                            <span class="badge text-uppercase bg-soft-primary text-primary rounded-pill">
                                                {{ \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $quotation->created_at)->format("F j, Y") }}
                                            </span>
                                        </td>
                                        {{-- <td class="text-end">
                        ...
                        <div class="card-footer border-0 py-5"><span class="text-muted text-sm">{{ $quotations->links() }}</span></div>
                    </div>
                </div>
            </div>
        </main>
